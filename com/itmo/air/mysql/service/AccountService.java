...
public class AccountService {
    ...
    public Result createAccount(Long userId, Account account) {
        if (userRepository.findById(userId).isPresent()) {
    ...
    public Result delete(Long userId, Long accountId, String secureWord) {
        if (userRepository.findById(userId).isPresent()) {
            User newUser = userRepository.findById(userId).get();
            Account newAccount = accountRepository.findById(accountId).get();
            List<Long> ids = new ArrayList<>();
    ...
    public Result logIn(Long userId, Account account) {
        if (userRepository.findById(userId).isPresent()) {
            User newUser = userRepository.findById(userId).get();
    ...
    public Result logOff(Long userId, Account account) {
        if (userRepository.findById(userId).isPresent()) {
            User newUser = userRepository.findById(userId).get();
            List<String> ids = new ArrayList<>();
            for (Account acc : newUser.getAccounts()) {
